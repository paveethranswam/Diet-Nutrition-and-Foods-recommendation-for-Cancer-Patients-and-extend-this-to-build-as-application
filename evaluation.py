    ...
    for i in range(preds.shape[0]):
        ...
        for p in pred:
            if p != 0:
                ...
                all_htr_embed.append(list(each_htr_np_embed))
                # print(' Relation between', title[i], h_idx[i], t_idx[i], p)
            # else: # lego added this
            #     print('No relation between', title[i], h_idx[i], t_idx[i], p)
    return res
...
def official_evaluate(tmp, path, train_file, dev_file):
    ...
    # print('lets see')
    # print(len(fact_in_train_annotated), len(fact_in_train_distant))
