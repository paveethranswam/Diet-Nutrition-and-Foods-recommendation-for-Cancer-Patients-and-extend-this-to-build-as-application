    ...
    def finetune(features, optimizer, num_epoch, num_steps, args):
        ...
        for epoch in train_iterator:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                #with torch.autograd.set_detect_anomaly(True):
                # lego added below 2 comment
                # with amp.scale_loss(loss, optimizer) as scaled_loss:
                #     torch.cuda.empty_cache()

                loss.backward()
                torch.cuda.empty_cache()
                # print('loss done here')
                gc.collect()
                ...
                if step % args.gradient_accumulation_steps == 0:
                    ...
                    torch.cuda.empty_cache()
                    if args.save_last != "":
                        print('Saving last trained stage as checkpoint from GAS ')
                        torch.save(model.state_dict(), args.save_last)
                        print('Last stage model saved from GAS')
                        torch.cuda.empty_cache()
                ...
                if (step + 1) == len(train_dataloader) - 1 or ((args.evaluation_steps > 0 and num_steps % args.evaluation_steps == 0 and step % args.gradient_accumulation_steps == 0) and num_steps > args.start_steps ):
                    if args.save_last != "":
                        print('Saving last trained stage as checkpoint ')
                        torch.save(model.state_dict(), args.save_last)
                        print('Last stage model saved')
                        torch.cuda.empty_cache()
                    ...
                    try:
                        ...
                        print('Error at evaluation')
                        if args.save_path != "":
                            print('Saving model ')
                            torch.save(model.state_dict(), args.save_path)
                            print('Model saved')
                    # Lego moving model save from here to before evaluate
                    # if args.save_last != "":
                    #     torch.save(model.state_dict(), args.save_last)
                    #     torch.cuda.empty_cache()
    ...
    for batch in dataloader:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                pred[np.isnan(pred)] = 0
                preds.append(pred)
                each_preds = list(pred)
                logits.append(logit.detach().cpu())
                # Lego added this to do preds official at every batch
                print('done')
                # preds_batch = np.concatenate(each_preds, axis=0).astype(np.float32)
                # preds_batch = to_official(preds_batch, features)
                torch.cuda.empty_cache()



        except Exception as e:
            print('error at', count, e)
            continue
    
    print('done with report')
    global all_htr_embed
    print(len(all_htr_embed))
    torch.save(all_htr_embed,'htr_embed_batch')

    preds = np.concatenate(preds, axis=0).astype(np.float32)
    preds = to_official(preds, features)
    print('offical', len(preds), preds)
...
def main():
    ...
    parser.add_argument("--num_class", type=int, default=97, # lego change from 97 to 2
    ...
    if os.path.exists(os.path.join(args.data_dir, args.test_file + suffix)):
        ...
        print('Created and saved new train features')
    n_e = 50 # lego all n_e from 42 to 50
    ...
    # lego adding here to decrease model parameters - also change config line
    ...
    set_seed(args)
    # Lego adding this line to reduce hidden size
    ...
    if args.load_pretrained != "": #Training from checkpoint
        ...
        # f1: 13.09, ign:0, dev_P: 10.97, dev_R: 16.21
        ...
        # model = amp.initialize(model, opt_level="O0", verbosity=0)
        # model.load_state_dict(torch.load(args.load_path), strict=False) # lego commented this
        model.load_state_dict(torch.load(args.load_path, map_location='cpu'), strict=False) # I added this to make it work (load to cpu and not gpu)
        # dev_score, dev_output = evaluate(args, model, dev_features, label_loader, tag="dev")
        # print(dev_output)
        print('enter test')
        # Lego also commented the next lines till else 
        pred, logits = report(args, model, test_features, label_loader)
        with open(args.output_name, "w") as fh:
            json.dump(pred, fh)
